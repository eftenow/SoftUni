...
class Television(RCA, HDMI, PowerOutlet):
...
class DVDPlayer(HMI, PowerOutlet):
...
class GameConsole(HDMI, EthernetCable, PowerOutlet):
    def connect_to_tv(self, television):
        self.connect_to_device_via_hdmi_cable(television)
...
class Router(EthernetCable, Television, GameConsole):
    def connect_to_tv(self, television):
